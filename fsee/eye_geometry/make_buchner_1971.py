...
from fsee.eye_geometry.util import get_mean_interommatidial_distance
from fsee.eye_geometry.make_receptor_info import find_edges, \
     make_receptor_sensitivities, flatten_cubemap, cube_order, \
     make_repr_able
import ashelf
import sys
...
weight_maps = make_receptor_sensitivities( receptor_dirs,
...
if 1:
    ...
    tmp_weights = flatten_cubemap( weight_maps[0] ) # get first one to take size

    n_receptors = len(receptor_dirs)
    len_wm = len(tmp_weights)

    print 'allocating memory...'
    bigmat = numpy.zeros( (n_receptors, len_wm), dtype=floattype )
    print 'done'

    print 'flattening, clipping, casting...'
    for i, weight_cubemap in enumerate(weight_maps):
        weights = flatten_cubemap( weight_cubemap )
        if clip_thresh is not None:
            weights = numpy.choose(weights<clip_thresh,(weights,0))
        bigmat[i,:] = weights.astype( bigmat.dtype )
    print 'done'

    print 'worst gain (should be unity)',min(numpy.sum( bigmat, axis=1))
    print 'filling spmat...'
    sys.stdout.flush()
    ...
    M,N = bigmat.shape
    print 'Compressed to %d of %d'%(len(spmat.data),M*N)
...
def main():
    ...
    fd = open('precomputed_buchner_1971.py','wb')
    fd.write( '# Automatically generated by make_buchner_1971.py\n')
    ...
    save_as_python(fd, receptor_dir_slicer, 'receptor_dir_slicer',
                   fname_extra='_buchner_1971' )
    save_as_python(fd, edge_slicer, 'edge_slicer',
                   fname_extra='_buchner_1971' )
    save_as_python(fd, spmat, 'receptor_weight_matrix', fname_extra='_buchner_1971' )
    save_as_python(fd, map(make_repr_able,receptor_dirs), 'receptor_dirs', fname_extra='_buchner_1971' )
