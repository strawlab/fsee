...
while time_now < duration:
    ...
    stimC1 = sinC_contrast*math.sin( time_now*2*math.pi*sinC_TF )
    stimC2 = sinC_contrast*math.sin( time_now*2*math.pi*sinC_TF + math.pi/2 )

    photoreceptors = [stimA1, stimA2, stimB1, stimB2, stimC1, stimC2 ]
...
emdB = trad_results[:,2]
emdC = trad_results[:,3]
emdAa_bd = trad_results[:,4]
emdBa_bd = trad_results[:,5]
emdCa_bd = trad_results[:,6]
emdAad_b = trad_results[:,7]
emdBad_b = trad_results[:,8]
emdCad_b = trad_results[:,9]
stimA1 = trad_results[:,10]
stimA2 = trad_results[:,11]
stimB1 = trad_results[:,12]
stimB2 = trad_results[:,13]
stimC1 = trad_results[:,14]
stimC2 = trad_results[:,15]

stimA1u = trad_results[:,16]
stimA2u = trad_results[:,17]
stimB1u = trad_results[:,18]
stimB2u = trad_results[:,19]
stimC1u = trad_results[:,20]
stimC2u = trad_results[:,21]

stimA1d = trad_results[:,22]
stimA2d = trad_results[:,23]
stimB1d = trad_results[:,24]
stimB2d = trad_results[:,25]
stimC1d = trad_results[:,26]
stimC2d = trad_results[:,27]
...
if 0:
    ...
    emdCe_func = CSplineInterp(times,rectify(emdCa_bd))
    emdAi_func = CSplineInterp(times,rectify(emdAad_b))
    emdBi_func = CSplineInterp(times,rectify(emdBad_b))
    emdCi_func = CSplineInterp(times,rectify(emdCad_b))
...
if 1:
    ...
    pylab.legend()
    ax2=pylab.subplot(4,1,2,sharex=ax,sharey=ax)
    ...
    pylab.legend()
    ax2=pylab.subplot(4,1,3,sharex=ax,sharey=ax)
    Ce = supersample(emdCe_func)
    Ci = supersample(emdCi_func)# sign flipped to show negative driving potential
    pylab.plot( newtimes, Ce, label='Ce' )
    pylab.plot( newtimes, Ci, label='Ci' ) 
    pylab.plot( newtimes, Ce-Ci, label='Ce-Ci' )
    pylab.legend()
    pylab.draw()
...
def objective_func(params):
    ...
    emdCe_func.gain=gscale
    emdCi_func.gain=gscale
    cptC = Compartment(emdCe_func,
                       emdCi_func,
                       name='cpt C')
    cptC.Cm = CmAB
    cptC.g_leak = g_leakAB
    cptC.link( cptX, g_a )

    cpts=[cptX,cptA,cptB,cptC]
